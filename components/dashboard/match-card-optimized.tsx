...
/**
 * Performance-optimized MatchCard component with React.memo
 * Only re-renders when props actually change
 */
export const MatchCard = memo<MatchCardProps>(function MatchCard({ 
  match, 
  getInitials, 
  getLastMessage, 
  getLastMessageTime,
  ...
  const memberCities = React.useMemo(() => 
    [...new Set(match.match_members.map(m => m.profiles.city))].join(', '),
    [match.match_members]
  )

  const memberSpecialties = React.useMemo(() => 
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-4 mb-4">
              <div className="flex -space-x-3">
                {match.match_members.slice(0, 5).map((member, index) => (
                  <Avatar key={`${member.user_id}-${index}`} className="w-10 h-10 border-3 border-white shadow-lg">
            ...
            <div className="bg-gray-50/50 rounded-lg p-3 mb-4">
              ...
              <div className="text-sm text-gray-500">
                ...
                {lastMessage}
                {lastMessageTime && (
                  <span className="ml-2 text-blue-500">• {lastMessageTime}</span>
                )}
              </div>
            </div>
          </div>
