...
export default function ModernNav({ showSearch = false, transparent = false, className = "" }: ModernNavProps) {
  ...
  const baseClasses = transparent 
    ? "fixed top-0 left-0 right-0 z-50 border-b border-white/10 backdrop-blur-xl bg-white/5 supports-backdrop-blur:bg-white/5 supports-no-backdrop-blur:bg-white/90 transition-all duration-300"
    : "sticky top-0 z-50 border-b border-white/20 backdrop-blur-xl bg-white/80 supports-backdrop-blur:bg-white/60 supports-no-backdrop-blur:bg-white/95 shadow-lg shadow-violet-500/5"
        ...
        <nav className="flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-10">
            {navItems.map((item) => (
              <Link 
                key={item.href} 
                href={item.href} 
                className="text-gray-700 hover:text-emerald-600 font-semibold text-lg transition-all duration-300 relative group px-3 py-2 rounded-lg hover:bg-white/20 hover:backdrop-blur-sm"
              >
                {item.label}
          ...
          <div className="flex items-center gap-3">
            ...
            <Button 
              variant="ghost" 
              size="sm" 
              className="hidden lg:flex items-center gap-2 text-gray-600 hover:text-emerald-600 hover:bg-white/30 hover:backdrop-blur-sm transition-all duration-300 rounded-lg px-3 py-2"
            >
