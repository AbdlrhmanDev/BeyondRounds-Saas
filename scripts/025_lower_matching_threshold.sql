...
-- ========================================
-- This script lowers the matching threshold from 0.55 to 0.3 to allow groups to form

-- Drop and recreate get_valid_pairs with lower threshold
...
BEGIN
    FOR user1_record IN SELECT * FROM get_eligible_users() LOOP
        FOR user2_record IN SELECT * FROM get_eligible_users() WHERE user_id > user1_record.user_id LOOP
            ...
            -- LOWERED THRESHOLD: 0.3 instead of 0.55
            IF score >= 0.3 THEN
...
-- Also update create_groups_greedy to use lower threshold
    ...
    LOOP
        ...
        FOR expansion_round IN 1..2 LOOP
            ...
            FOREACH potential_user IN ARRAY eligible_users_for_expansion LOOP
                ...
                -- LOWERED THRESHOLD: 0.3 instead of 0.55
                IF test_score >= 0.3 AND test_score > best_score THEN
        ...
        IF array_length(current_group, 1) >= 3 AND 
           calculate_group_average_score(current_group) >= 0.3 AND
...
SELECT 
    'Testing with lowered threshold (0.3)' as status,
...
-- Now test the matching with lower threshold
SELECT trigger_manual_matching();

-- Success message
SELECT 
    '✅ Threshold lowered to 0.3!' as status,
    'Groups should now be created' as message;
