...
/**
 * BeyondRounds TypeScript Types Migration Script
 * 
 ...
 * 
 * Key transformations:
 * - profiles → users (table names)
 * - Profile → User (type names)
 * - non-binary → non_binary (enum standardization)
 * - Import path updates
 * - Form data type alignment
 */
...
const REGEX_PATTERNS = [
  ...
  // Type imports
  { pattern: /import\s*{\s*([^}]*Profile[^}]*)\s*}\s*from\s*['"][^'"]*database['"]/gi, replacement: (match: string, types: string) => {
